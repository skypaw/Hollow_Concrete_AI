...
def draw(doc, i, a, h, as1, a1, r, l, base_x, base_y):
    ...
    #   override={'dimtoh': 1}).render()
        ...
        msp.add_arc(
            center=(base_x + a / 2, base_y + h / 2 + l / 2),
            radius=round(r, 2),
            start_angle=360,
            end_angle=180,
        )
...
def visualize_models():
    dimensions_csv = np.genfromtxt("full_data.csv", delimiter=",")
    ...
    try:
        for i in range(rows):
            for j in range(rows * 2):
                if draw_iterator + 1 in dimensions_csv[:, 0]:
                    draw(
                        doc,
                        dimensions_csv[draw_iterator, 0],
                        dimensions_csv[draw_iterator, 1],
                        dimensions_csv[draw_iterator, 2],
                        dimensions_csv[draw_iterator, 3],
                        dimensions_csv[draw_iterator, 4],
                        dimensions_csv[draw_iterator, 5],
                        dimensions_csv[draw_iterator, 6],
                        base_x,
                        base_y,
                    )
                else:
        ...
        doc.saveas("models_visualization_without_dimensions.dxf")
    except:
        print("daasdf")
    finally:
