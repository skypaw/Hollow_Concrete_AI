...
class Model:
    ...
    def check_dimensions(self):
        ...
        self._r, self._l, self._a1 = m_functions.check_dimensions(
            self._r, self._l, self._a, self._a1, self._h
        )
    ...
    def _materials(self):
        ...
        self._model_parameters.materials[self._steel_material_name].Elastic(
        ...
        self._model_parameters.materials[self._concrete_material_name].Elastic(
    ...
    def _section_create(self):
        ...
        self._model_parameters.HomogeneousSolidSection(
            name="Section-concrete",
            material=self._concrete_material_name,
            thickness=None,
        )

        self._model_parameters.TrussSection(
            name="Section-reinforcement",
    ...
    def _step_create(self):
        ...
        self._model_parameters.StaticStep(
            name="Step-1",
            previous="Initial",
            maxNumInc=10000,
            initialInc=1,
    ...
    def _model_assembly(self):
        ...
        p = self._model_parameters.parts[self._reinforcement_part_name.format("1")]

        a1.Instance(name="SteelRod-1", part=p, dependent=ON)
        a1 = self._model_parameters.rootAssembly
        a1.rotate(
            instanceList=("SteelRod-1",),
            axisPoint=(0.0, 1.0, 0.0),
            axisDirection=(0.0, -1.0, 0.0),
            angle=90.0,
        )

        a1 = self._model_parameters.rootAssembly
        p = self._model_parameters.parts[self._reinforcement_part_name.format("2")]

        a1.Instance(name="SteelRod-2", part=p, dependent=ON)
        a1 = self._model_parameters.rootAssembly
        a1.rotate(
            instanceList=("SteelRod-2",),
            axisPoint=(0.0, 1.0, 0.0),
            axisDirection=(0.0, -1.0, 0.0),
            angle=90.0,
        )
        ...
        a1 = self._model_parameters.rootAssembly
        a1.translate(
            instanceList=("SteelRod-1",), vector=(self._a - self._a1, self._a1, 0.0)
        )
        a1.translate(instanceList=("SteelRod-2",), vector=(self._a1, self._a1, 0.0))

        a1 = self._model_parameters.rootAssembly
        a1.rotate(
            instanceList=("SteelRod-1", "SteelRod-2", "ConcreteCube-1"),
    ...
    def _constraint_set(self):
        ...
        cells1 = c1.getSequenceFromMask(
            mask=("[#1 ]",),
        )
        region2 = regionToolset.Region(cells=cells1)
        self._model_parameters.EmbeddedRegion(
            name="Constraint-1",
    ...
    def _section_assigment(self):
        ...
        p.SectionAssignment(
            region=region,
            sectionName="Section-reinforcement",
            offset=0.0,
    ...
    def _job_create(self):
        ...
        self._model_database.Job(
            name=self._job_name.format(self._i),
    ...
    def _job_write_and_change_input_calculation(self):
        ...
        self._model_database.jobs[self._job_name.format(self._i)].writeInput(
            consistencyChecking=OFF
        )
        self._model_database.jobs[self._job_name.format(self._i)].waitForCompletion()

        c_input.change_input(
            self._job_name.format(self._i), job_name_changed.format(self._i)
        )

        del self._model_database.jobs[self._job_name.format(self._i)]
        self._model_database.JobFromInputFile(
            name=job_name_changed.format(self._i),
        ...
        self._model_database.jobs[job_name_changed.format(self._i)].submit(
            consistencyChecking=OFF
        )
    ...
    def _part_concrete_cube(self):
        ...
        if self._l == 0:
            s = self._model_parameters.ConstrainedSketch(
                name="__profile__", sheetSize=self._a
            )
            s.setPrimaryObject(option=STANDALONE)

            s.rectangle(point1=(0.0, 0.0), point2=(self._a, self._h))
            s.CircleByCenterPerimeter(
                center=(self._a / 2, self._h / 2),
                point1=(self._a / 2, self._h / 2 - self._r),
            )

            self._model_parameters.Part(
                name=self._cube_part_name, dimensionality=THREE_D, type=DEFORMABLE_BODY
            )
            ...
            s.Line(
                point1=(left_point_line, down_point_line),
                point2=(left_point_line, up_point_line),
            )
            s.Line(
                point1=(right_point_line, down_point_line),
                point2=(right_point_line, up_point_line),
            )

            s.ArcByCenterEnds(
                center=(self._a / 2, up_point_line),
                point1=(left_point_line, up_point_line),
                point2=(right_point_line, up_point_line),
                direction=CLOCKWISE,
            )
            s.ArcByCenterEnds(
                center=(self._a / 2, down_point_line),
                point1=(right_point_line, down_point_line),
                point2=(left_point_line, down_point_line),
                direction=CLOCKWISE,
            )
    ...
    def _part_steel_rod(self):
        ...
        self._model_parameters.Part(
            name=self._reinforcement_part_name,
    ...
    def _create_model_database(self):
        ...
        self._model_database.models.changeKey(
            fromName="Model-1", toName=self._model_name
        )
        self._model_parameters = self._model_database.models[
            self._model_name.format(self._i)
        ]
